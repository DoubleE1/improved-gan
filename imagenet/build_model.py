...
def build_model(self):
    ...
    all_g_grads = []
    config = self.config
    d_opt = tf.compat.v1.train.AdamOptimizer(config.discriminator_learning_rate, beta1=config.beta1)
    g_opt = tf.compat.v1.train.AdamOptimizer(config.generator_learning_rate, beta1=config.beta1)
...
def build_model_single_gpu(self, gpu_idx):
    ...
    if gpu_idx == 0:
        filename_queue = tf.compat.v1.train.string_input_producer([filename]) #  num_epochs=self.config.epoch)
        ...
        with tf.compat.v1.variable_scope("misc"):
            ...
            avg_error_rate = tf.compat.v1.get_variable('avg_error_rate', [],
                    initializer=tf.constant_initializer(0.),
                    trainable=False)
            num_error_rate = tf.compat.v1.get_variable('num_error_rate', [],
                    initializer=tf.constant_initializer(0.),
                    trainable=False)

    images, sparse_labels = tf.compat.v1.train.shuffle_batch([self.get_image, self.get_label],
    ...
    if gpu_idx == 0:
        self.sample_images= tf.compat.v1.placeholder(tf.float32, [self.sample_size] + self.image_shape,
                                        name='sample_images')
        self.sample_labels = tf.compat.v1.placeholder(tf.int32, [self.sample_size], name="sample_labels")
    ...
    d_loss_class = class_loss_weight * tf.nn.sparse_softmax_cross_entropy_with_logits(class_logits,
            tf.compat.v1.to_int64(sparse_labels))
    error_rate = 1. - tf.reduce_mean(tf.compat.v1.to_float(tf.nn.in_top_k(class_logits, sparse_labels, 1)))
    # self.d_loss_class = tf.Print(self.d_loss_class, [error_rate], "gpu " + str(gpu_idx) + " current minibatch error rate")
    if gpu_idx == 0:
        update = tf.compat.v1.assign(num_error_rate, num_error_rate + 1.)
        ...
        update = tf.compat.v1.assign(avg_error_rate, (1. - tc) * avg_error_rate + tc * error_rate)
        with tf.control_dependencies([update]):
