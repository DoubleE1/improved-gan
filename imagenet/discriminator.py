...
def discriminator(self, image, reuse=False, y=None, prefix=""):
    ...
    with tf.compat.v1.variable_scope("d_diff_feat"):
    ...
    noisy_image = image + tf.compat.v1.random_normal([batch_size, 128, 128, 3],
            mean=0.0,
            stddev=.1)
    ...
    gan_logits = tf.compat.v1.log(tf.reduce_sum(tf.exp(safe_pos_class_logits), 1)) + tf.squeeze(mx) - generated_class_logits
